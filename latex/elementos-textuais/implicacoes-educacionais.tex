...
A geração de dados pelo modelo físico simulado pelo jogo e a posterior coleta permitida pelo software contemplam duas práticas com o mesmo nome, catalogadas pela taxonomia como ``Prática relacionada a dados'' (subseção \ref{sub-sec:pratica-de-dados}). O julgamento qualitativo do modelo decorrente do valor descoberto para a constante gravitacional insere-se na categoria ``Prática de simulação e modelagem'' (subseção \ref{praticas-de-simulacao-e-modelagem}) no item ``Avaliação de modelos computacionais'.
