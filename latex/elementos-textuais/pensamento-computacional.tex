...
Nesta etapa, entra em cena o conceito de modelagem: um processo de identificação de agentes\footnote{entidades físicas ou sociais, sejam elas um grupo de elétrons ou de indivíduos} e interações, bem como a atribuição de variáveis para as quantidades definidas como importantes para descrição. 

O modelo, é por essência, uma expressão da escolha daquilo que é considerado relevante. Uma redução da realidade. Posto em outras palavras, ele nada mais é do que uma representação do problema ou fenômeno. Uma abstração descrita em termos de outras menores: seus agentes, as relações entre eles, e os processos que os envolvem. E é justamente esse arranjo que nos permite enxergar essas unidades como módulos. 

Ser capaz de expressar seus aspectos em termos de problemas computacionais exige que as perguntas a serem respondidas sejam estabelecidas de modo que computadores possam respondê-las a partir da formulação de algoritmos, cuja implementação por uma linguagem de programação resulta nos programas, ou ``máquinas'' que automatizam a solução buscada. 
