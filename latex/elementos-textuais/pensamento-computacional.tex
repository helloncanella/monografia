...
% %
% Afinal como essas elementos -- a construção de algoritmos, problemas computacionais, e a noção de abstração -- se articulam para compor o pensamento computacional?
...
Nesta etapa, entra em cena o conceito de modelagem: um processo de identificação de agentes\footnote{entidades físicas ou sociais, sejam elas um grupo de elétrons ou de indivíduos} e interações, bem como de atribuição de variáveis, cujas quantidades são consideradas relevantes para descrição. 
...
% A possibilidade de automatizar um modelo através de uma simulação 

% A automação do modelo é o que permite 

% A perspectiva do arranjo de camadas de abstração


% O arranjo em módulos, apontado acima se estrutura nessa etapa. 


% A criação e arranjo em módulos, como apontado acima, é essencial nessa etapa. Principalmente quando 

% % exige que as perguntas a serem respondidas sejam estabelecidas de modo que computadores possam respondê-las a partir da formulação de algoritmos, cuja implementação por uma linguagem de programação resulta nos programas, ou ``máquinas'' que automatizam a solução buscada. 




% % OS MODULOS REPRESENTANDO OS ELEMENTOS QUE COMPOEM O MODELO DEVEM TAMBÈM SER REPRESENADOS PELOS PGRAMA.
...
% % Ao implementarmos os algoritmos com o uso de uma linguagem de programação e das estruturas de dados ofercidas por ele O uso de linguagem de programação, por seu turno, ao 


% % O processo de modelagem, assim sendo, pode ser entendida  Modelar nada mais é do que o ato de criar abstrações.

% % O PROBLEMA COMPUTACIONAL DEVE COMPORTAR PERGUNTAS PONTENCIALMENTE 


% % de processos  variáveis matemáticas, e processos 
